package plugins.globalstrings;

public class GlobalStrings {
	// Plugin
	public static final String PLUGIN_LABEL_TOLOWERCASE = "To Lower Case";
	public static final String PLUGIN_LABEL_TOUPPERCASE = "To Upper Case";
	public static final String PLUGIN_LABEL_REMOVESVOWELS = "Removes voyels";
	public static final String PLUGIN_LABEL_CAESARCIPHER = "Caesar cipher %d";

	// Frame
	public static final String FRAME_TITLE = "Extendable Editor";

	// Menu
	public static final String MENU_TOOLS = "Tools";
}
